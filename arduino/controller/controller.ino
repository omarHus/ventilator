...
#define MIN_VALVE_CURRENT   (50) // mA
...
{    
    ...
    // change frequency of valve pins to avoid signal noise
    analogWriteFrequency(PWM_PATIENT_1_VALVE, 585937.5);
    analogWriteFrequency(PWM_PATIENT_2_VALVE, 585937.5);
...
//        debug_double("\tPatient 2 Pressure Sensor = %s PSI", p2);
        ...
        msg.pressure_p1 = PSI_TO_CMH2O * abs(p1-p_atm);
        msg.pressure_p2 = PSI_TO_CMH2O * abs(p2-p_atm);
...
//        debug_double("\tPatient 2 Duty Cycle = %s percent", 100.0 * (duty_cycle_p2 / 255.0));            
        ...
        comms_send(&msg);
//        serial_plotter(msg.setpoint_p1, msg.flow_p1, duty_cycle_p1);
...
double flowMeasurement(double inlet_pressure, double p1)
{
    ...
    // Outlet speed
    double v_outlet = sqrt(abs(delta_P)/(1-square(A_Ratio)));

    // Calculate Flow rate (check if positive or negative)
    double myflow_rate = (delta_P < 0) ? 0.0 : A_2*v_outlet*M3_S_TO_ML_MIN;
...
//    debug_double("\terror = %s", error);
//    debug_double("\trate error = %s", rateError);
//    debug_double("\tsum error = %s", *sumError);

    // add bias so that valves are never fully closed
    double bias = 0.0;
    
    // Compute PID output
    return PID_GAIN_P*error + PID_GAIN_I*(*sumError) + PID_GAIN_D*rateError + bias;
}
...
{  
    ...
    if (pid_output >= MAX_VALVE_FLOW_RATE) {
        // largest change
       delta = 25;
    } 
    else if (pid_output <= -MAX_VALVE_FLOW_RATE) {
       delta = -25;
    }
    else {
        ...
        delta_current = map(pid_output, -MAX_VALVE_FLOW_RATE, MAX_VALVE_FLOW_RATE, MIN_VALVE_CURRENT, MAX_VALVE_CURRENT);
//        debug_double("\tMapped Current = %s", delta_current);
...
void serial_plotter(double setpoint, double flow1, double duty_cycle_p1){
